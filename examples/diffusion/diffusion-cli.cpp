...
int main(int argc, char ** argv) {
    ...
    // infill: the prompt IS the canvas (mask markers inside it are the holes) - single shot, used
    // verbatim (no chat template, no conversation loop), and the whole canvas is printed
    if (diff_params.infill) {
        std::vector<llama_token> canvas = common_tokenize(vocab, params.prompt,
                                                          /*add special*/ true, /*parse special*/ true);
        const int n_input = (int) canvas.size();
        if (n_input > (int) params.n_ubatch) {
        ...
        diffusion_generate(ctx, canvas.data(), output_tokens.data(), n_input, diff_params, n_generated);
        if (n_generated > 0) {
            output_tokens.resize(n_generated);
            LOG("\n%s\n", common_detokenize(vocab, output_tokens, false).c_str());
        } else {
