...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    ).set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_DIFFUSION}).set_env("LLAMA_ARG_HOST"));
    add_opt(common_arg(
        {"--port"}, "PORT",
    ...
    ).set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_DIFFUSION}).set_env("LLAMA_ARG_PORT"));
    add_opt(common_arg(
        {"--reuse-port"},
    ...
    add_opt(common_arg(
        {"--diffusion-kv-span"}, "F",
        string_format("dynamic kv-block sizing: confidence span threshold (0 = fixed blocks) (default: %.2f)", (double) params.diffusion.kv_span),
    ...
    add_opt(common_arg(
        {"--diffusion-gen-initial"}, "N",
        string_format("in-run canvas growth: start the active canvas at N tokens and grow on demand (0 = off) (default: %d)", params.diffusion.gen_initial),
        [](common_params & params, int value) { params.diffusion.gen_initial = value; }
    ).set_examples({ LLAMA_EXAMPLE_DIFFUSION }));
    add_opt(common_arg(
